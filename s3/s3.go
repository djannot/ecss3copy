...
/*
Copy - copy objects in another bucket using the directive indicated
Added by Denis Jannot
*/
func (b *Bucket) CopyToNewBucket(oldPath, newPath, sourceBucketName string, perm ACL, directive string) error {
	if !strings.HasPrefix(oldPath, "/") {
		oldPath = "/" + oldPath
	}
	...
	req := &request{
		...
		path:   newPath,
		headers: map[string][]string{
			"x-amz-copy-source": {amazonEscape("/" + sourceBucketName + oldPath)},
			"x-amz-acl": {string(perm)},
			"x-amz-metadata-directive": {string(directive)},
		},
	}
...
func (b *Bucket) Query(query, marker string, max int) (result *QueryResp, err error) {
	params := map[string][]string{
		"query":    {query},
	}
	if marker != "" {
